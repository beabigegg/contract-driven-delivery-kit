...
- A green gate/oracle test — or a loosened check — proves nothing without mutation-corpus evidence that no defect escapes (stronger agents earn more freedom, never more self-certified trust: self-review, self-acceptance, or a parity verdict of `equivalent` with no mutation corpus); in gate tests assert the stream (`log.warn`→stdout, `log.error`→stderr), not the exit code — see `contracts/ci/ci-gate-contract.md` "Pass/fail shape", `docs/loosening-the-harness.md`.
