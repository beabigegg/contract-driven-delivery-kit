...
applicability-reason: "CLI/CI-infrastructure change to the cdd-kit tool itself with no user-facing screen, control, or visual state. It fixes exit-code/enforcement semantics of `cdd-kit boundary check`, a base-resolution bug in `src/boundary/guard.ts`, and a bash-loop robustness bug in the shipped GitHub Actions workflow template. The only observable surfaces are a CLI exit code and stdout/stderr log lines (both governed by contracts/ci/ci-gate-contract.md), not an interaction-design surface."
---
