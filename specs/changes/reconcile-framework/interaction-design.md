...
applicability-reason: "CLI-only upgrade-tooling change to the cdd-kit tool itself: a dry-run/plan-mode surface classifier + reconciliation registry + a new upgrade-reconciliation contract. Its only observable surfaces are CLI plan-mode stdout and file dispositions, governed by contracts/upgrade/upgrade-reconciliation-contract.md and contracts/ci/ci-gate-contract.md — no user-facing screen, control, or visual state."
---
