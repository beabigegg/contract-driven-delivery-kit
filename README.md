...
- **Test evidence.** For implementation changes, `test-evidence.yml` must exist with its required phases passed (collect, targeted, changed-area, plus any declared contract/quality/full), no waiver fields, and each run pointing at this change's `test-runs/`. Generated by `cdd-kit test run`; opt out a change with no testable surface via `test-evidence-not-applicable: "<reason>"` in `tasks.yml` frontmatter. After a migration window this is enforced — a warning for legacy changes, an error for context-governed changes or under `--strict`.
...
cdd-kit test run <change-id> --phase collect
cdd-kit test run <change-id> --phase targeted --json
cdd-kit test run <change-id> --phase changed-area
cdd-kit test run <change-id> --phase full
```

`test run` options: `--phase <phase>` (required), `--command <cmd>` (until
`select` provides one; pytest commands get bounded defaults `-q --maxfail=1
--tb=short -ra` plus JUnit XML), `--run-id <id>`, `--timeout <ms>` (default
300000), `--cwd <dir>`, `--required-phases <csv>` (the floor used when first
creating evidence; default `collect,targeted,changed-area`), and `--json`.
