...
- `test-evidence.yml` present for implementation changes, with the required
  phases passed (collect, targeted, changed-area, plus any applicable
  contract/quality/full), `final-status: passed` and no recorded run failed
  (even a non-required full smoke -- the gate blocks on any failed run), no
  waiver fields, and each run pointing at this change's `test-runs/` --
  generated by `cdd-kit test run`, not hand-authored. A change with no testable
