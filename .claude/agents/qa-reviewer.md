...
- `test-evidence.yml` present for implementation changes, with the required
  ...
  contract/quality/full), no waiver fields, and each run pointing at this
  change's `test-runs/` -- generated by `cdd-kit test run`, not hand-authored.
  A change with no testable code surface must carry
