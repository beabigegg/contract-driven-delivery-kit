...
1. **Declare a typed schema** under `## Schemas`: a `| field | type | required |` table (Tier A — primitives, `enum(...)`, schema refs, `[]`) or a fenced ` ```json-schema ` block (Tier B — needed for nested objects / arrays of objects). Point the endpoint's `response schema` cell at that schema name. **The cell must be the BARE schema name** — `AckResponse` or `AckResponse[]`, nothing else. A decorated cell (`→ AckResponse`, `see AckResponse`, `AckResponse (success)`) does **not** resolve to a `$ref`; cdd-kit treats it as prose and the body stays unenforced. `cdd-kit openapi export` and `doctor` warn on this near-miss, and a sampled near-miss is a hard error in `validate --contracts`.
