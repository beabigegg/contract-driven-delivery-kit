...
| `interaction-design.md` | screen information, user intents, control↔intent mapping, states + discriminators, human confirmation (ADR 0012); `applicability: not-applicable` + reason when the change has no UI surface — and then DELETE the rest of the file, the gate reads only the marker |

`change-classification.md`, `test-plan.md`, and `ci-gates.md` are the OLD
(`context-governance: v1`) shape and are no longer scaffolded or required. Writing
one by hand for a new change produces an orphan nothing reads. Existing v1 change
directories keep them and are never migrated by hand.
...
- `spec.md` ≤ 200 lines
- `design.md` ≤ 150 lines
- `implementation-plan.md` ≤ 250 lines, of which `## Test Plan` ≤ 100 and `## CI Gates` ≤ 80
...
**`## Test Plan` (in `implementation-plan.md`) should contain:**
...
Changes that modify code also produce `test-evidence.yml` under
`specs/changes/<id>/`: the bounded, machine-readable record of which test phases
ran (collect, targeted, changed-area, contract, quality, full) and their
results. It is generated by `cdd-kit test run`, not hand-authored -- see
`specs/templates/test-evidence.yml` for the shape and `implementation-plan.md`'s
`## Test Plan` section for the execution ladder. It is the durable test evidence for the change. A required
test failure cannot be recorded as known, pre-existing, waived, allowed, or
...
`## Optional Artifacts` output explicitly marks them as `yes`, or when a reviewer
finds blocking findings / approved-with-risk evidence that must be preserved as prose.

`design.md` is owned by `spec-architect`, not `implementation-planner`. If
`tasks.yml` marks `design.md` as `yes` — that is, `classification.architecture-review`
is `true`, or `classification.required-agents` lists `spec-architect` — invoke
`spec-architect` before `implementation-planner`. If neither trigger is present,
mark task `1.3` as `skipped`.
...
**After updating `tasks.yml`'s classification**: read the classifier's `## Tasks Not Applicable` list. For each listed task ID (e.g., `2.2`, `4.2`), update `tasks.yml` to change that item's `status:` from `pending` to `skipped`. Do this before invoking any other agent.

---

## Step 3: Read the tier and commission agents

Read `tasks.yml` (top-level `tier:` and the `classification:` block) to determine the tier. Then invoke agents **in the exact order below**.
...
If the classifier's `## Lane` output is `bug-fix`, the request is
...
3. **`test-strategist`** (write-capable) — writes the `## Test Plan` section of `specs/changes/<change-id>/implementation-plan.md` directly.
...
4. **`spec-architect`** (write-capable) — only if `tasks.yml`'s `classification.architecture-review` is `true`, or the classifier lists `spec-architect` in `## Required Agents`.
...
5. **`ci-cd-gatekeeper`** (write-capable) — writes the `## CI Gates` section of `specs/changes/<change-id>/implementation-plan.md` directly before implementation planning.
   - YOU tick: `1.4`, `4.4`, applicable items in section 6

6. **`implementation-planner`** (write-capable) — writes the remaining sections of `specs/changes/<change-id>/implementation-plan.md` directly once classification, the `## Test Plan` and `## CI Gates` sections, and required design are available.
   ...
   - It must reference this file's own `## Test Plan` / `## CI Gates` sections, contracts, and `design.md` by path/section/id instead of copying their full content.
...
9. **`dependency-security-reviewer`** (read-only) — if the change touches lockfiles, package manifests, or DB migrations.
   - **Only invoke if** the classifier's output lists lockfiles, package manifests, or DB migrations as affected.
...
| any `implementation-plan.md: … "## Test Plan" …` line — the three the gate emits are `missing or empty "## Test Plan" section`, `` `## Test Plan` has no acceptance-criterion → test row ``, and `` `## Test Plan` still holds only the scaffold `` (an untouched v2 scaffold produces the middle one, not the first) | `test-strategist` | "PREVIOUS TEST PLAN FAILED GATE: <error>. Re-fill the `## Test Plan` section of `implementation-plan.md` — the criterion→test mapping table must name at least one acceptance criterion and the test that covers it." |
| any `implementation-plan.md: … "## CI Gates" …` line — either `missing or empty "## CI Gates" section` or `` `## CI Gates` still holds only the scaffold `` (an untouched v2 scaffold produces the second) | `ci-cd-gatekeeper` | "PREVIOUS CI GATE PLAN FAILED GATE: <error>. Re-fill the `## CI Gates` section of `implementation-plan.md` with real gate rows, not the blank scaffold." |
| `context-manifest.md: …` | `change-classifier` | "PREVIOUS MANIFEST FAILED GATE: <error>. Re-emit `## Context Manifest Draft`." |
| `tasks.yml: …` (any other frontmatter / pending error — the classification-substance row above takes precedence when both match) | YOU (main Claude) — direct edit | n/a — fix `tasks.yml` yourself. Don't re-invoke an agent for a file you own. |
...
- Never skip the `## Test Plan` section of `implementation-plan.md` for Tier 0–1 changes
- Never skip the `## CI Gates` section of `implementation-plan.md` for any implementation change
