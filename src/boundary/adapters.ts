...
print(json.dumps({'body': body, 'status': response.status_code, 'content_type': content_type, 'runner_version': version}))
`;
...
console.log(JSON.stringify({body: response.body, status: response.status, content_type: String(response.headers['content-type'] || '').split(';', 1)[0], runner_version: version}));
`;
...
export function executeRegisteredCapture(
  cwd: string,
  operation: Pick<BoundaryOperation, 'method' | 'path'>,
  ...
  const parsed = JSON.parse(output) as Omit<AdapterCaptureResult, 'target_digest'>;
  if (!Number.isInteger(parsed.status) || typeof parsed.runner_version !== 'string') throw new Error(`Boundary adapter returned an invalid envelope for ${variant.id}.`);
  return { ...parsed, target_digest: captureTargetDigest(operation, variant) };
}
